...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/availability":
            ...
            for t in SLOT_TIMES:
                ...
                slots.append({"time": t, "available": fits_hours and not conflict})
            return self.send_json(200, {"slots": slots, "reason": None, "closeTime": min_to_time(close_min)})
    ...
    def do_POST(self):
        ...
        if path == "/api/staff/settings":
            ...
            conn = db.get_conn()
            conn.execute(
                "UPDATE salon_settings SET open_time=?, close_time=?, closed_weekdays=?, updated_at=? WHERE id=1",
                (open_time, close_time, closed_str, db.now_iso()),
            )
            conn.commit()
            conn.close()
            return self.send_json(200, {"ok": True})
