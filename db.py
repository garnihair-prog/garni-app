...
_COLOR_CONSENT_ARTICLES = [
    ...
    ("第9条（免責事項）",
     "施術によって、かぶれ・アレルギー反応・頭皮・毛髪のダメージ等の症状や損害が発生した場合であっても、"
     "その原因を問わず、当店は一切の責任を負わないものとする。"),
    ("第10条（個人情報の取扱い）",
...
_PERM_CONSENT_ARTICLES = [
    ...
    ("第7条（免責事項）",
     "施術によって、かぶれ・アレルギー反応・熱傷等の症状や損害が発生した場合であっても、"
     "その原因を問わず、当店は一切の責任を負わないものとする。"),
    ("第8条（個人情報の取扱い）",
...
def init_db():
    ...
    # 同意書の本文（body_html）はスタッフダッシュボードから編集する手段が無く、常にこのファイルの
    # 内容が正となる。カラー・パーマ同意書の文面を更新した場合、既存DBにも反映されるよう、
    # 起動のたびに最新の内容へ同期する（差分がなければ実質的に何も変わらない）。
    conn.execute(
        "UPDATE consent_forms SET body_html=? WHERE id='cf-color'", (COLOR_CONSENT_HTML,)
    )
    conn.execute(
        "UPDATE consent_forms SET body_html=? WHERE id='cf-perm'", (PERM_CONSENT_HTML,)
    )
