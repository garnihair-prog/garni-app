...
let companions = [];   // 複数人でのご来店（お連れ様）：{ id, name, menuIds: Set<string> } の配列
let companionSeq = 0;  // お連れ様カードのDOM用ID採番（表示順の識別のみに使用、サーバーには送らない）
...
// 実際にお名前とメニューの両方が入力されているお連れ様のみ、予約対象として扱う
function validCompanions() {
  return companions.filter(c => c.name.trim() && c.menuIds.size > 0);
}

function companionMenuRowHtml(comp, m) {
  const checked = comp.menuIds.has(m.id) ? "checked" : "";
  return `
    <label class="companion-menu-row">
      <input type="checkbox" ${checked} onchange="toggleCompanionMenu('${comp.id}','${m.id}')">
  ...
  return `
    <div class="companion-card" id="companion-${comp.id}">
      ...
      <div class="companion-menu-list">
        ${MENUS.map(m => companionMenuRowHtml(comp, m)).join("")}
      </div>
    </div>`;
}

function renderCompanions() {
  document.getElementById("companion-list").innerHTML = companions.map(c => companionCardHtml(c)).join("");
}
...
function totalDurationMin() {
  const primary = [...selectedMenus].reduce((sum, id) => {
    const m = MENUS.find(x => x.id === id);
    return sum + (m ? m.duration_min : 0);
  }, 0);
  const companionsTotal = validCompanions().reduce((sum, c) => {
    return sum + [...c.menuIds].reduce((s, id) => {
      const m = MENUS.find(x => x.id === id);
      return s + (m ? m.duration_min : 0);
    }, 0);
  }, 0);
  return primary + companionsTotal;
}
...
async function renderSlots() {
  ...
  const comps = validCompanions();
  const combinedMenuIds = [...selectedMenus, ...comps.flatMap(c => [...c.menuIds])];
  const menuIds = combinedMenuIds.join(",");
  document.getElementById("time-duration-hint").textContent =
    `合計所要時間：約${duration}分` + (comps.length ? `（お連れ様${comps.length}名を含む）` : "");
  try {
...
function renderDisclaimer() {
  ...
  document.getElementById("disclaimer-body").innerHTML = `
    ...
    ご予約の変更・キャンセルはお早めにご連絡ください。土日祝のご予約について、前日のキャンセルは予約金額の${feePercent}%、当日キャンセルは${feePercentFull}%のキャンセル料を申し受ける場合がございます。無断キャンセルの場合は、平日・土日祝を問わず、毎回予約金額の${feePercentFull}%のキャンセル料を申し受けます。また、ご来店が5分以上遅れる場合は、恐れ入りますが当店までご連絡ください。10分以上遅れた場合は、誠に勝手ながらご予約をキャンセルさせていただく場合がございます。</p>
...
function renderConfirm() {
  ...
  const primaryTotal = names.reduce((a, m) => a + m.price, 0);
  const hasFromPrice = names.some(m => m.price_is_from);
  const totalStudentDiscount = names.reduce((a, m) => a + (m.student_discount || 0), 0);

  const companionRows = validCompanions().map(c => {
    const cMenus = [...c.menuIds].map(id => MENUS.find(m => m.id === id)).filter(Boolean);
    return {
      name: c.name.trim(),
      menuNames: cMenus.map(m => m.name).join("・"),
      price: cMenus.reduce((a, m) => a + m.price, 0),
      hasFromPrice: cMenus.some(m => m.price_is_from),
    };
  });
  ...
  document.getElementById("confirm-summary").innerHTML = `
    ...
    <div class="row total"><span>合計</span><span>¥${grandTotal.toLocaleString()}${anyFromPrice ? "〜" : ""}</span></div>
    ${anyFromPrice ? `<div class="row"><span></span><span style="font-size:11px;color:var(--text-muted);">※目安料金を含みます。実際の料金は状態により変動します</span></div>` : ""}
...
function downloadBookingICS() {
  ...
  let menuNames = names.map(m => m.name).join("・") || "ご予約";
  const comps = validCompanions();
  if (comps.length) menuNames += `（お連れ様${comps.length}名）`;
