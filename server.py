...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/staff/settings":
            ...
            conn.execute(
                "UPDATE salon_settings SET open_time=?, close_time=?, closed_weekdays=?, combo_perm_color_last_order=?, cancellation_fee_percent=?, updated_at=? WHERE id=1",
                (open_time, close_time, closed_str, combo_last_order, fee_percent, db.now_iso()),
            )
            conn.commit()
            conn.close()
            return self.send_json(200, {"ok": True})
    ...
    def do_PATCH(self):
        ...
        if m:
            ...
            if not resv:
                ...
                return self.send_json(404, {"error": "not found"})
            cancellation_fee = 0
            if status == "cancel" and is_weekend_or_holiday(resv["date"]):
                settings = get_settings(conn)
                fee_percent = settings.get("cancellation_fee_percent", 50)
                cancellation_fee = round(resv["total_price"] * fee_percent / 100)
